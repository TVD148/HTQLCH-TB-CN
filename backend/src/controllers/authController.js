...
const register = async (req, res, next) => {
  try {
    ...
    // Tách tên / họ tự động từ họ tên đầy đủ
    ...
    const [result] = await db.query(
      `INSERT INTO nguoi_dung (ho_ten, ten, ho, email, mat_khau_ma_hoa, so_dien_thoai, dia_chi)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [name, ten, ho, email, mat_khau_ma_hoa, phone || null, address || null]
    );
